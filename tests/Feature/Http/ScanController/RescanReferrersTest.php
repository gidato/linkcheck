...
class RescanReferrersTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function a_rescan_referrer_pages_can_be_triggered_for_site()
    {
        ...
        $response = $this->post('/scans/'.$this->scan->id.'/rescan-referrers');
        $response->assertRedirect();

        $this->assertCount(2, Scan::all());
        $scan = Scan::where('id','<>',$this->scan->id)->first();
        $this->assertEquals(20, $scan->pages->count());  // error page is dropped
        $this->assertEquals(19, $scan->pages->where('checked',1)->count()); // referring one has been set to not checked
